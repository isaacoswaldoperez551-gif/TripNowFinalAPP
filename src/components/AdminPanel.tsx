...
import {
  ...
  Activity, Radio, Clock, UserCheck, Car, Search, Filter, UserX
} from 'lucide-react';
...
interface AdminPanelProps {
  ...
  onDeleteClient?: (id: string, name?: string, email?: string) => Promise<void> | void;
  onDeleteAllClients?: () => Promise<void> | void;
  onRefreshData: () => void;
}
...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  // Client Management & Deletion states
  ...
  // Listen for global "tripnow_open_delete_clients" events from Navbar or shortcuts
  React.useEffect(() => {
    const handleOpen = () => {
      setActiveTab('clients');
      setShowDeleteManager(true);
    };
    window.addEventListener('tripnow_open_delete_clients', handleOpen);
    return () => window.removeEventListener('tripnow_open_delete_clients', handleOpen);
  }, []);

  // Handle single client delete confirmation
  const handleConfirmDeleteClient = async () => {
    if (!clientToDelete) return;
    setIsDeletingClient(true);
    try {
      if (onDeleteClient) {
        await onDeleteClient(clientToDelete.id, clientToDelete.nombre, clientToDelete.email);
      }
      toast.success(
        lang === 'es' ? 'Cliente eliminado' : 'Client removed',
        lang === 'es' ? `${clientToDelete.nombre} eliminado de Firebase y del sistema.` : 'Removed from database.'
      );
  ...
  // Handle wipe all clients
  const handleConfirmDeleteAllClients = async () => {
    setIsDeletingClient(true);
    try {
      if (onDeleteAllClients) {
        await onDeleteAllClients();
      }
      toast.success(
        lang === 'es' ? 'Clientes eliminados' : 'Clients purged',
        lang === 'es' ? 'Se han eliminado todos los clientes de Firebase.' : 'All clients removed from database.'
      );
      setShowDeleteAllModal(false);
      setShowDeleteManager(false);
    } catch (err) {
      toast.error('Error', 'No se pudieron eliminar los clientes.');
    } finally {
      setIsDeletingClient(false);
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-stone-200">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          {/* Botón en la Barra Admin Superior para Eliminar Clientes */}
          <button
            onClick={() => {
              setActiveTab('clients');
              setShowDeleteManager(true);
            }}
            className="px-3.5 py-2 bg-rose-600 hover:bg-rose-700 active:scale-95 text-white text-xs font-semibold rounded-lg transition-all flex items-center gap-1.5 cursor-pointer shadow-sm"
            title="Eliminar clientes en Firebase"
          >
            <Trash2 className="w-4 h-4 text-white" />
            <span className="font-mono">Eliminar Clientes</span>
          </button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="bg-white p-4 sm:p-5 rounded-2xl border border-stone-200 shadow-sm col-span-2 lg:col-span-1 flex flex-col justify-between">
          <div>
          ...
          <button
            ...
            className="mt-2 text-[11px] text-rose-600 hover:text-rose-800 font-mono flex items-center gap-1 cursor-pointer font-semibold"
          >
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 border-b border-stone-200 scrollbar-none">
        ...
        >
          ...
          {clients.length > 0 && (
            <span className="bg-stone-200 text-stone-800 text-[10px] font-mono px-1.5 py-0.2 rounded-full">
              {clients.length}
            </span>
          )}
        </button>

        {/* Botón directo en la barra de navegación admin de pestañas */}
        <button
          onClick={() => {
            setActiveTab('clients');
            setShowDeleteManager(true);
          }}
          className="px-3.5 py-2 text-xs font-bold rounded-lg whitespace-nowrap transition-all flex items-center gap-1.5 cursor-pointer bg-rose-600 hover:bg-rose-700 text-white shadow-xs"
          title="Eliminar clientes de Firebase"
        >
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-mono uppercase tracking-wider text-amber-800 font-semibold">
                  Firebase Firestore · Colección `clients`
                </span>
              ...
              <h2 className="font-serif text-xl sm:text-2xl font-bold text-stone-900 mt-1">
                {lang === 'es' ? 'Directorio y Control de Clientes' : 'Client Directory & Management'}
              </h2>
              <p className="text-xs text-stone-500 font-mono mt-0.5">
                {lang === 'es'
                  ? 'Gestiona y elimina clientes registrados de Firebase Firestore y del sistema.'
                  : 'Manage and delete registered clients from Firebase Firestore and system.'}
              </p>
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {clients.length > 0 && (
                <button
                  ...
                  className="px-3.5 py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 text-xs font-medium rounded-lg transition-colors flex items-center gap-1.5 cursor-pointer shadow-xs"
                  title="Eliminar todos los clientes de Firebase"
                >
                  <UserX className="w-3.5 h-3.5 text-rose-600" />
                  <span>Vaciar Todos los Clientes</span>
                </button>
              )}
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded-2xl border border-stone-200 shadow-sm flex flex-col sm:flex-row gap-3 items-center justify-between">
            <div className="relative flex-1 w-full">
              ...
              <input
                ...
                placeholder={lang === 'es' ? 'Buscar cliente por nombre, correo, teléfono o DUI...' : 'Search by name, email, phone...'}
            ...
            {clientSearch && (
              <button
                ...
                className="text-xs font-mono text-stone-500 hover:text-stone-900 px-2 py-1 cursor-pointer"
              >
                Limpiar filtro
              </button>
            )}
              ...
              <table className="w-full text-left text-xs font-mono">
                <thead className="bg-stone-50 border-b border-stone-200 text-stone-500 uppercase">
                  <tr>
                    ...
                    <th className="p-4">Documento / DUI</th>
                  ...
                  {clients
                    ...
                    <tr>
                      <td colSpan={7} className="p-8 text-center text-stone-500">
                        {clients.length === 0
                          ? 'No hay clientes registrados en la base de datos de Firebase.'
                          : 'No se encontraron clientes con el filtro ingresado.'}
                      </td>
                    </tr>
                  ) : (
                    clients
                      ...
                      .map(c => (
                        <tr key={c.id} className="hover:bg-stone-50/80 transition-colors">
                          <td className="p-4 font-bold text-stone-900 text-[11px] font-mono">
                            {c.id}
                          </td>
                          <td className="p-4">
                          ...
                          <td className="p-4 text-stone-600 text-[11px]">
                            {c.documento || <span className="text-stone-400 italic">No especificado</span>}
                          </td>
                          ...
                          <td className="p-4 text-right">
                            <button
                              ...
                              className="px-2.5 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded-lg text-xs font-medium transition-colors cursor-pointer inline-flex items-center gap-1.5"
                              title={`Eliminar ${c.nombre} de Firebase`}
                            >
      ...
      {/* Modal: Confirm Delete Single Client */}
      {clientToDelete && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl border border-stone-200">
            ...
            <p className="text-xs text-stone-600 font-mono mb-4 leading-relaxed">
              Esta acción borrará de forma permanente el documento de <strong className="text-stone-900">{clientToDelete.nombre}</strong> en tu base de datos de Google Firestore y en la aplicación.
            </p>

            <div className="bg-stone-50 rounded-xl p-3 border border-stone-200 text-xs font-mono space-y-1 mb-6 text-stone-700">
              <div><strong>ID:</strong> {clientToDelete.id}</div>
              <div><strong>Correo:</strong> {clientToDelete.email}</div>
              <div><strong>Teléfono:</strong> {clientToDelete.telefono}</div>
              {clientToDelete.documento && <div><strong>DUI / Licencia:</strong> {clientToDelete.documento}</div>}
            </div>
            ...
            <div className="flex items-center justify-end gap-2">
              <button
                ...
                className="px-4 py-2 bg-stone-100 hover:bg-stone-200 text-stone-700 text-xs font-medium rounded-lg cursor-pointer transition-colors"
              >
                Cancelar
              </button>
              <button
                ...
                className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-medium rounded-lg cursor-pointer transition-colors flex items-center gap-1.5 shadow-sm"
              >
                {isDeletingClient ? (
                  <>
      ...
      {/* Modal: Confirm Delete All Clients */}
      {showDeleteAllModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl border border-stone-200">
            <div className="w-12 h-12 rounded-2xl bg-rose-100 text-rose-600 flex items-center justify-center mb-4">
            ...
            <h3 className="font-serif text-xl font-bold text-stone-900 mb-2">
              ¿Vaciar todos los clientes de Firebase?
            </h3>
            
            <p className="text-xs text-stone-600 font-mono mb-6 leading-relaxed">
              Se eliminarán <strong className="text-rose-600">{clients.length} cliente(s)</strong> de la colección <code className="bg-stone-100 px-1 py-0.5 rounded">clients</code> en tu base de datos de Google Firebase. Esta acción es irreversible.
            </p>
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <button
                type="button"
                disabled={isDeletingClient}
                onClick={handleConfirmDeleteAllClients}
                className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-medium rounded-lg cursor-pointer transition-colors flex items-center gap-1.5 shadow-sm"
              >
                {isDeletingClient ? (
                  <>
                    <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                    <span>Vaciando Firebase...</span>
                  </>
                ) : (
                  <>
                    <Trash2 className="w-3.5 h-3.5" />
                    <span>Sí, Vaciar Todos</span>
                  </>
                )}
              </button>
            </div>
      ...
      {/* Modal: Quick Delete Manager Drawer/Modal from Admin Bar */}
      {showDeleteManager && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl border border-stone-200 max-h-[90vh] flex flex-col">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-serif text-lg font-bold text-stone-900">
                    Administrador de Clientes · Firebase
                  </h3>
                  <p className="text-xs text-stone-500 font-mono">
                    Selecciona un cliente para borrarlo de Firebase en tiempo real
                  </p>
                </div>
              </div>

              <button
                onClick={() => setShowDeleteManager(false)}
            ...
            <div className="my-4">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full text-xs font-mono bg-stone-50 border border-stone-200 rounded-lg pl-9 pr-3 py-2 text-stone-900 focus:outline-none focus:ring-2 focus:ring-rose-500"
                />
              </div>
            </div>

            {/* List of clients */}
            <div className="flex-1 overflow-y-auto divide-y divide-stone-100 pr-1 space-y-1 max-h-80">
              {clients.length === 0 ? (
                ...
                clients
                  .filter(c => {
                    if (!clientSearch.trim()) return true;
                    const s = clientSearch.toLowerCase();
                    return c.nombre.toLowerCase().includes(s) || c.email.toLowerCase().includes(s) || c.telefono.toLowerCase().includes(s);
                  })
                  .map(c => (
                    <div key={c.id} className="py-3 px-3 hover:bg-stone-50 rounded-xl flex items-center justify-between gap-3 transition-colors">
                      <div className="min-w-0">
                        <div className="font-bold text-xs text-stone-900 font-sans truncate">{c.nombre}</div>
                        <div className="text-[11px] text-stone-500 font-mono truncate">{c.email} · {c.telefono}</div>
                        {c.documento && <div className="text-[10px] text-stone-400 font-mono">DUI: {c.documento}</div>}
                      </div>
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 text-xs font-medium rounded-lg transition-colors cursor-pointer flex items-center gap-1 shrink-0"
                      >
                        <Trash2 className="w-3.5 h-3.5 text-rose-600" />
                        <span>Eliminar</span>
                      </button>
            ...
            <div className="pt-4 border-t border-stone-200 flex items-center justify-between gap-2 mt-2">
              {clients.length > 0 ? (
                <button
                  onClick={() => setShowDeleteAllModal(true)}
                  className="px-3 py-2 bg-stone-100 hover:bg-rose-50 text-stone-700 hover:text-rose-700 text-xs font-mono rounded-lg transition-colors flex items-center gap-1.5 cursor-pointer"
                >
                  <UserX className="w-3.5 h-3.5" />
                  <span>Vaciar todos ({clients.length})</span>
                </button>
              ) : <div />}

              <button
                onClick={() => setShowDeleteManager(false)}
                className="px-4 py-2 bg-stone-900 hover:bg-stone-800 text-white text-xs font-medium rounded-lg cursor-pointer transition-colors"
              >
                Cerrar
              </button>
            </div>
